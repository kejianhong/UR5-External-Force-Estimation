...
int main(int argc, char **argv){
    ...
    description.add_options()("help,h", "Turn the gain parameter for external observer.")
    ...
    ("frequency,f", boost::program_options::value<double>()->default_value(100), "sample Frequency")
    ("observerType,t", boost::program_options::value<int>(), "observer type:\n \
    0: momentum observer\n \
    ...
    q.setZero();qd.setZero();tau.setZero();tauExtMeasured.setZero(); tauExtEstimated.setZero();
    // 画图
    ...
    for (unsigned int index = 0; index < dof; ++index)
    {
        ...
        gp << "plot '-' with linespoints linecolor "
        << to_string(index + 1)
        << " linewidth 0.6 pointtype "
        << to_string(index + 1)
        << " pointsize 1.0 title \"measured_"
        << to_string(index + 1) << "\" axis x1y1,"
        << "'-' with linespoints linecolor "
        << to_string(index + 2)
        << " linewidth 0.6 pointtype "
        << to_string(index + 2)
        << " pointsize 1.0 title \"estimated_"
        << to_string(index + 1) << "\" axis x1y1,"
        << "'-' with linespoints linecolor "
        << to_string(index + 3)
        << " linewidth 0.6 pointtype "
        << to_string(index + 3)
        << " pointsize 1.0 title \"vel_"
        << to_string(index+1) << "\" axis x1y2\n";
